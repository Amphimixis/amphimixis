...
> **Note:** For readability, only the most significant rows per event are shown, with `[unknown]` symbols removed. See the complete tables 10-row tables in [Usage Examples](docs/usage_examples.md).
...
> **What do `0.00%` mean?**
`0.00%` values means the symbol exists only on one architecture (e.g. `__strncmp_evex` AVX-512 on x86 vs scalar `strncmp` on RISC-V) or the event is not available on RISC-V (`cache-misses` is `cpu-clock` mapped).
