...
> **Note:** Tables below show only essential rows (5-9 per event, filtered by `|Delta%|`). `[unknown]` and one-sided `0.00%` entries are omitted for clarity to keep only functions with actionable insight. See the full honest 10-row tables in [Usage Examples](docs/usage_examples.md).
...
> **What do `0.00%` mean?**
`0.00%` means the symbol exists only on one architecture (e.g. `__strncmp_evex` AVX-512 on x86 vs scalar `strncmp` on RISC-V) or the event is not available on RISC-V (`cache-misses` is `cpu-clock` mapped). They appear in the full output but are filtered here to keep only meaningful, optimizable hotspots.
